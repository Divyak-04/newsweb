...
import { FaVolumeUp, FaArrowLeft, FaArrowRight, FaTimes } from "react-icons/fa";
...
const Home = () => {
  ...
  const [selectedIndex, setSelectedIndex] = useState(null); // Track selected news index
  ...
  <FaBars /> 
        ...
        <button
          ...
          className="text-3xl p-2 rounded-full shadow-md bg-red-300 dark:bg-gray-800 transition-all duration-300 hover:scale-110 mr-[80px]"
        >
          {darkMode ? <MdLightMode className="text-yellow-400" /> : <MdDarkMode className="text-gray-700" />}
        </button>
      </nav>
      <div className="pt-20">
      {/* Weather Display inside the page */}
...
{weather && (
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-6 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
        ...
        <motion.div whileHover={{ scale: 1.05 }} className="w-full md:w-1/4">
          <select
            ...
            className="w-full p-4 border-2 rounded-lg bg-white dark:bg-gray-800 dark:text-white text-gray-700 font-semibold shadow-sm"
            value={category}
  ...
  <input
    type="date"
    className="w-full p-3 border-2 rounded-lg shadow-sm bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-white font-semibold focus:outline-none focus:ring-2 focus:ring-blue-400"
    value={date}
  ...
  {menuOpen && (
    <motion.div
      ...
      className="fixed top-0 left-0 w-64 h-full bg-gray-800 text-white shadow-lg z-50 p-5"
    >
      {/* Close Button */}
      ...
      🗓 Published on: {new Date(article.publishedAt).toDateString()}
                ...
                <div className="flex justify-between  items-center p-3  bottom-0 left-0 bg-gray-100">
                    ...
                    <button
                      ...
                      className=" p-2 rounded-full bg-blue-400 text-white text-lg hover:bg-blue-700"
                    >
                      <FaVolumeUp />
                    </button>
                    ...
                    <button
                      ...
                      className=" ml-2 p-2 rounded bg-blue-400 text-white hover:bg-blue-700"
                    >
                      Enlarge
                    </button>
        ...
        {/* Share Button */}
        <button
          className="flex items-center justify-end bg-blue-400 text-white px-3 py-2 rounded hover:bg-blue-700"
          onClick={(e) => {
...
>
  <path
    fillRule="evenodd"
    d="M9.293 4.707a1 1 0 011.414 0L16.414 10a2 2 0 010 2.828l-5.707 5.293a1 1 0 11-1.414-1.414L14.586 12 9.293 6.707a1 1 0 010-1.414z"
    clipRule="evenodd"
  />
</svg>
        ...
        <div id="shareDropdown" className="absolute right-0 w-40 bg-white border rounded-md shadow-lg hidden z-50">
          <button
            ...
            className="block w-full text-left px-4 py-2 text-black-600 hover:bg-blue-300"
          >
            WhatsApp
          </button>
          <button
            onClick={() => shareNews("facebook", article.title, article.url)}
            className="block w-full text-left px-4 py-2 text-black-600 hover:bg-blue-300"
          >
            Facebook
          </button>
          <button
            onClick={() => shareNews("x", article.title, article.url)}
            className="block w-full text-left px-4 py-2 text-black-600 hover:bg-blue-300"
          >
            X
          </button>
        ...
        onClick={(e) => {
          ...
          e.currentTarget.closest(".relative").querySelector(".absolute").classList.add("hidden");
        }}
        className="block w-full text-left px-4 py-2 text-red-600 hover:bg-blue-300"
      >
        ❌ Close
      </button>
          ...
          >
            ...
            >
              ...
              <p className="text-justify text-lg leading-relaxed">
                {news[selectedIndex]?.content || "Full article content not available."}
              </p>
              ...
              <button
                ...
                onClick={() => setSelectedIndex((prev) => (prev > 0 ? prev - 1 : news.length - 1))}
              >
                <FaArrowLeft />
              </button>
              ...
              <button
                ...
                onClick={() => setSelectedIndex((prev) => (prev < news.length - 1 ? prev + 1 : 0))}
              >
                <FaArrowRight />
              </button>
            </motion.div>
